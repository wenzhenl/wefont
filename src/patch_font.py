...
def patch_font(old_font_file, patched_glyph_file, family_name):
    ...
    if family_name and family_name == font.familyname:
        raise Exception("Patched font family name must be different if you want to change it, you can keep it empty to keep the original family name")
    if family_name:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description=
        "patch existing font with new glyphs from template")
    parser.add_argument("font_file", help="path of the existing font")
    parser.add_argument("patched_glyph_file", help="path of the file with the list of patched glyphs")
    parser.add_argument("-fn",
                        "--family_name",
                        help="the family name of the patched font, keep the same as old font if not provided")
