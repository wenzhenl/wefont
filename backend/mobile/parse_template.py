...
# ////////// PARSE TEMPLATE /////
# //////////////////////////////////////////////
def parse_template( img, qrdata, verbose ):
    
    possible_centers = []
    detect_all_finders(img, possible_centers)

    if len(possible_centers) != len(qrdata) - 1:
        raise Exception("WRONG NUMBER OF FINDERS")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="parse template from a camera")
    ...
    while True:
        ...
        if not qrcode_detected:
            ...
            if qrdata and qrdata[0] == 20:
                qrcode_detected = True
                for i in qrdata:
                    print i
        
        try:
            if qrcode_detected:
                parse_template(img, qrdata, args.verbose)
        except:
            continue
